...
do_clean_linux_kernel()
{
    cd $LINUX_KERNEL_PATH
    make CROSS_COMPILE=aarch64-linux-gun- mrproper
    make CROSS_COMPILE=aarch64-linux-gun- clean O=build
    # make CROSS_COMPILE=aarch64-linux-gun- ${MACHINE}-linux_defconfig O=build
    # make CROSS_COMPILE=aarch64-linux-gun- O=build -j16
}
