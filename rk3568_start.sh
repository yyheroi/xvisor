...
Following are steps to create these images and boot Xvisor on the RockPro64:
  ...
  [13. Make U-Boot compatible udisk.img for use with bootm command] #和start_pc -a地址一样
  # mkimage -A arm64 -O linux -T ramdisk -a 0x20000000 -n "Xvisor Ramdisk" -d build/disk.img build/udisk.img
